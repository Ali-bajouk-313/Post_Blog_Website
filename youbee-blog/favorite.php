...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['post_id'])) {
    ...
    try {
        ...
        die("Error updating liked posts: " . $e->getMessage());
    }
}
            ...
            <div class="post-card">
                ...
                <div class="post-details">
                    ...
                    <div class="post-actions">
                        ...
                        <!-- Saved button -->
                        <button class="save-btn <?php echo hasUserSaved($post['id'], $user_id) ? 'saved' : ''; ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-width="1.8"
                                stroke-linecap="round" stroke-linejoin="round">
    ...
    <script>
        document.querySelectorAll('.save-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                const postId = this.closest('.post-card').querySelector('input[name="post_id"]')?.value || this.dataset.id;
                const button = this;
                ...
                fetch('save.php', {
                    ...
                    .then(res => res.text())
                    .then(response => {
                        button.classList.toggle('saved', response === 'saved');
                        fetch('get_save_count.php?post_id=' + postId)
                            .then(res => res.text())
